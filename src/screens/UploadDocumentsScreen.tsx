    ...
    const loadExisting = async () => {
      ...
      try {
        ...
        if (user?.kyc) {
          ...
          // Sabhi documents ko extract karne ke liye ek robust dynamic resolver loop
          const updatedImages = { ...images };
          const keys: DocumentKey[] = ['pan', 'aadhaar', 'salarySlip', 'itrFiling', 'bankStatement'];

          keys.forEach(key => {
            // Naming patterns ka complete automation (e.g. panUrl, panDocUrl, pan.url)
            const fieldName = DOCUMENT_META[key].apiField; 
            const directUrl = kyc[`${fieldName}Url`] || kyc[`${fieldName}DocUrl`] || kyc[fieldName]?.url;
            ...
            <View style={styles.formCard}>
              {fetching ? (
                <View style={{ paddingVertical: 40, alignItems: 'center' }}>
                  ...
                  <Text style={{ marginTop: 10, color: '#64748B' }}>Loading your profile documents...</Text>
                </View>
              ) : (
                <>
...
const styles = StyleSheet.create({
  ...
  formCard: { marginTop: -18, height: '150%', paddingTop: 20, shadowRadius: 18, paddingBottom: 32, shadowColor: '#000', shadowOpacity: 0.08, paddingHorizontal: 20, backgroundColor: '#fff', borderTopLeftRadius: 32, borderTopRightRadius: 32, shadowOffset: { width: 0, height: 10 } },
  ...
  pdfPreview: { marginTop: 8, height: 80, borderRadius: 12, borderWidth: 1, borderColor: '#E5E7EB', backgroundColor: '#F8FAFC', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, gap: 8 },
